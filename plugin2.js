...
function calculateAndDisplayRoute(box, path, directionsRenderer, markerArray, stepDisplay,map) {
	...
	.then((response) => {
		...
		showSteps(response, markerArray, stepDisplay,map,box);
    })
    .catch((e) => console.log("Directions request failed due to " + e));
}
function showSteps(directionResult, markerArray, stepDisplay,map,box) {
	let i = 0;
	...
	// 	);
	// 	map.setHeading(heading);
	// 	console.log("HEADING: " + heading + " LAT:" + mainMarker.getPosition().lat());
...
const GoogleMapsPluginApi = async (apikey, box, path) => {
	...
	const markerArray = [];
	const stepDisplay = new box.window.google.maps.InfoWindow();
	const directionsRenderer = new box.window.google.maps.DirectionsRenderer();
	// MAP CREATATION
	bg = mainBoard.querySelector(".bg");
	...
	`
    calculateAndDisplayRoute(box, path, directionsRenderer, markerArray,stepDisplay, map);
...
`
let bg = mainBoard.querySelector(".bg");
